...
def run_pipeline(
    ...
    try:
        ...
        # Only runs if Phase 1 produced styled images (GPU neural model).
        # Without a styled image, retouching the raw original adds no value
        # and the Modal endpoint would 500 trying to fetch non-existent paths.
        # ─────────────────────────────────────────────────
        has_styled_images = any(r.get("processed_img") is not None for r in analysis_results)
        modal_base = __import__("os").environ.get("MODAL_BASE_URL", "")

        if has_styled_images and modal_base:
            log.info("Phase 2: Face retouching via GPU (Modal)")
            set_job_phase(processing_job_id, "retouch", 0)
            # TODO: Wire Modal face_retouch calls here once training data is available
            # For now, fall through to the basic CPU sharpening below
            for i, result in enumerate(analysis_results):
            ...
            log.info("Phase 2: Retouching (basic CPU sharpen — no GPU model available)")
            set_job_phase(processing_job_id, "retouch", 0)
            for i, result in enumerate(analysis_results):
                img = result.get("processed_img")
                if img is not None and settings.get("retouch_intensity", "light") != "off":
